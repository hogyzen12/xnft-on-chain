...
def get_replacement_indices(matched_index, total_matches, is_horizontal, candy_images_length):
    if matched_index == SPECIAL_CANDY_INDEX:  # Special card
        if is_horizontal:
            # For horizontal matches, use 5, 4, 3 in the middle
            return [5, 4, 3]
        else:
            # For vertical matches, use 0, 1, 2 in the middle
            return [0, 1, 2]
    else:
        if total_matches == 4:
            ...
            # and the sequence depends on the direction of the match
            previous_index = matched_index - 1 if matched_index - 1 >= 0 else candy_images_length - 2
            next_index = (matched_index + 1) % (candy_images_length - 1)

            if is_horizontal:
                # For horizontal matches, use 5, 4, 3 in the middle
                middle_indices = [5, 4, 3]
            else:
                # For vertical matches, use 0, 1, 2 in the middle
                middle_indices = [0, 1, 2]

            return [previous_index] + middle_indices + [next_index]
        else:
            previous_index = matched_index - 1 if matched_index - 1 >= 0 else candy_images_length - 2
            next_index = (matched_index + 1) % (candy_images_length - 1)
...
def detect_and_replace_matches(board):
    ...
    def replace_candies(start_row, start_col, length, is_horizontal):
        ...
        indices = get_replacement_indices(candy_type, length, is_horizontal, CANDY_IMAGES_COUNT)
        for i in range(length):
            if is_horizontal:
                board[start_row, start_col + i] = indices[i % length]
            else:
                board[start_row + i, start_col] = indices[i % length]
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 validate.py <seed> <moves_string>")
        sys.exit(1)

    seed = sys.argv[1]
    moves_string = sys.argv[2]
    #print(seed)
    
    points, cards_collected = calculate_score(seed, moves_string)
    # Print the results in a single line, separated by a comma
